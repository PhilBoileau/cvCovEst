...
are constructed by regularizing the sample covariance matrix, or throught the
estimation of latent factors. A comprehensive review is provided by @fan:2016.
...
(high-dimensional) asymptotic optimality of selections are guaranteeed by
adapting the seminal work of @laan-dudoit:2003, @dudoit2005, and @dudoit2006 on
data-adaptive estimator selection to high-dimensional covariance matrix
...
$p$-length random vectors with possibly nonparametric data-generating
...
function. Depeding on your assumptions -- or lack thereof -- about the true
...
data-generating distribtion, and the goal of the estimation procedure. This
...
Like the candidate estimators, `cvCovEst` only supports loss functions
implemented in this package. Please submit suggestions to
[issue](https://github.com/PhilBoileau/cvCovEst/issues).
...
| Monte-Carlo | To perform Monte-Carlo cross-validation, set the `cv_scheme` argument to `"mc"`, and set the proprotion of data to be used in each validation set using the `mc_split` argument. |
