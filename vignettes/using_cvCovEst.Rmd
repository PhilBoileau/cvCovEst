...
`list` of class `cvCovEst`, a `dat_orig` argument, the original data used to calculate the covariance matrix estimators, and a `summ_fun` argument, a character vector
...
| Best Performing Estimator by Class | `bestInClass` | Returns the specific hyperparameters, if applicable, of the best performing estimator within each class along with additional estimator metrics. |
| Worst Performing Estimator by Class | `worstInClass` | Returns the specific hyperparameters, if applicable, of the worst performing estimator within each class along with additional estimator metrics. | 
...
When either `bestInClass` or `worstInClass` is specified, the additional estimator metrics are the condition number, the sign, and the sparsity.  Sign refers to the matrix sign and is one of positive-definite (`"PD"`), positive-semi-definite ("PSD"), negative-definite ("ND"), negative-semi-definite ("NSD"), or indefinite ("IND").  If the estimator results in the zero matrix, then the sign is returned as `"NA"`.  Sparsity is calculated at the proportion of total matrix entries which are estimated to be 0.
